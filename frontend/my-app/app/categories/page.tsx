      ...
      <section className="bg-card py-16 border-b border-border">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            <h1 className="text-4xl md:text-5xl font-bold mb-4 text-card-foreground">News Categories</h1>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-muted-foreground" suppressHydrationWarning />
              <Input
                placeholder="Search categories or topics..."
                className="pl-10 bg-input border-border text-foreground placeholder-muted-foreground focus:border-ring h-12 text-lg"
              />
            </div>
          </div>
        </div>
      ...
      <div className="bg-card border-b border-border">
        <div className="container mx-auto px-4 py-4">
          <Tabs value={activeTab} onValueChange={handleTabChange}>
            <TabsList className="bg-card border-border flex-wrap h-auto p-2 w-full justify-start">
      ...
      <div className="container mx-auto px-4 py-12">
              ...
              {categories.length > 0 ? (
                categories.map((category) => (
                  <div key={category.slug} onClick={() => handleTabChange(category.slug)}>
                    <Card className="bg-card border-border hover:bg-card/90 transition-all duration-200 h-full cursor-pointer group">
              ...
              {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {Array.from({ length: 6 }).map((_, i) => (
                    <div key={i} className="h-64 bg-card rounded animate-pulse"></div>
                  ))}
                </div>
              ) : articles.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-8">
                ...
                <div className="bg-card rounded-lg p-6 border border-border">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h1 className="text-3xl font-bold mb-2 text-card-foreground">{category.name}</h1>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-primary">{category.article_count}</div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold text-foreground">Latest {category.name} Articles</h2>
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <Clock className="h-4 w-4" />
                      <span className="text-sm">Updated recently</span>
                    </div>
        ...
        <section>
          <Card className="bg-card border-border">
            <CardHeader>
              <CardTitle className="text-card-foreground text-center">Platform Statistics</CardTitle>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-center">
                <div>
                  <div className="text-3xl font-bold text-primary mb-2">{stats.total_articles.toLocaleString()}</div>
                ...
                <div>
                  <div className="text-3xl font-bold text-primary mb-2">{stats.accuracy_rate}%</div>
                  <div className="text-muted-foreground text-sm">Accuracy Rate</div>
                </div>
                <div>
                  <div className="text-3xl font-bold text-primary mb-2">24/7</div>
                  <div className="text-muted-foreground text-sm">Fact Checking</div>
                </div>
              </div>
            </CardContent>
          </Card>
